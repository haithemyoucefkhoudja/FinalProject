...
export default  function  PopUpEditCompany (props:{ company_name: string; send: () => void, updateCompanyName:(newName:string)=>void }) {
  const [LocalCompanyName , setLocalCompanyName] = useState(props.company_name);
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const values = {
      ...
      new_company_name: LocalCompanyName,
    };
    ...
    if (result.success) {
      ...
      props.send()
      props.updateCompanyName(LocalCompanyName)
    } else {
            ...
            <div className="bg-white rounded-lg shadow-md p-6 w-1/2 max-w-md">
              ...
              >
                ...
                <form onSubmit={handleSubmit}>
                  <input onChange={event=>setLocalCompanyName(event.target.value)} placeholder={props.company_name} className=" mb-[1rem] w-full rounded-sm ring-2 ring-gray-200" type="text" />
