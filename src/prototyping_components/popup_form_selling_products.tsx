...
export default function Popup_form_selling_products(props:any){
...
return(
 ...
 <p className="w-[40%]  ">product {index+1} {item.name}</p><input type="number" placeholder="0" className="w-[40%] bg-white border black-border rounded outline-none"/>
 <p className="w-[40%]  text-center">{item.available_stock}</p> 
 </div>)})
return (
    <>
   <div className="fixed top-[0] left-[0] w-[100vw] h-[100vh] bg-black bg-opacity-20" onClick={handleClick}></div>
    
    <div className="flex flex-col justify-center items-center text-black w-[40vw] h-[40vh] rounded-[1rem] p-[1rem] border black-border absolute top-[50%] left-[50%] transform translate-x-[-50%] translate-y-[-50%] bg-white"> 
      <p className="fixed top-[1%] right-[1%] text-[1.5em] text-[red]" onClick={handleClick}>X</p>
          
          <p className="absolute left-[37%] top-[5%]  h-[50%] ">Sell products</p>
          <div className="flex w-[85%]  justify-end"><p className="mr-[1rem]">available stock</p></div>
